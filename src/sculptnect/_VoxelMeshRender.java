...
public class VoxelMeshRender {
	...
	private static final int CHUNK_SIZE = 32;// make sure power of two otherwise markVoxelDirty method fails
	...
	private class ChunkData {
		...
		FloatBuffer vertexBuffer = ByteBuffer
			.allocateDirect(
				...
				(CHUNK_SIZE + VERTEX_GRID_OFFSET*2) *
				(CHUNK_SIZE + VERTEX_GRID_OFFSET*2) * ITEM_STRIDE * 3)// from every vert 6 tris can originate, so should be *6
					...
					for (int x=chunk.lowerIndices[0]; x<chunk.upperIndices[0]; ++x)
						for (int y=chunk.lowerIndices[1]; y<chunk.upperIndices[1]; ++y)
							for (int z=chunk.lowerIndices[2]; z<chunk.upperIndices[2]; ++z)
								updateVertex(chunk, faces, x, y, z);
					
					// if normals havent been calced, do it here
					// normals must be normalized, either do it here or make sure GL_NORMALIZE is enabled
...
//		else if (mZ == 1 && z<grid.d-CHUNK_SIZE) dirtyChunks.add(chunks[ix][iy][iz+1]);
	...
	public void updateDirtyChunks (GL2 gl) {
		...
		try {
			while (true) {
				...
				gl.glBufferData(GL.GL_ARRAY_BUFFER, chunk.numIndices * ITEM_STRIDE, chunkData.vertexBuffer, GL.GL_STATIC_DRAW);
	...
	/** Calcs and appends normal to v0, v1, v2, when all tris for a chunk have
	 *  been calculated all vertices will be traversed and normalized
	 */
//	private static void addNormals (float[] v0, float[] v1, float[] v2) {
//		// cross product, get triangle normal
//		float x = (v1[1]-v0[1])*(v2[2]-v0[2]) - (v1[2]-v0[2])*(v2[1]-v0[1]);
//		float y = (v1[2]-v0[2])*(v2[0]-v0[0]) - (v1[0]-v0[0])*(v2[2]-v0[2]);
//		float z = (v1[0]-v0[0])*(v2[1]-v0[1]) - (v1[1]-v0[1])*(v2[0]-v0[0]);
//		// add triangle normal to all vertices
//		v0[3] += x; v0[4] += y; v0[5] += z;
//		v1[3] += x; v1[4] += y; v1[5] += z;
//		v2[3] += x; v2[4] += y; v2[5] += z;
//	}
	
	private static void addFace (ArrayList<float[]> faces, float[] v0, float[] v1, float[] v2) {
		faces.add(v0);
		faces.add(v1);
		faces.add(v2);
	}
	
	private void updateVertex (Chunk chunk, ArrayList<float[]> faces, int x, int y, int z) {
		/*
	...
	private void setNormal (int x, int y, int z, float[] v) {
		// Iterate through the 124 neighbors of the voxel
		final int min = -2;// -2
		final int max = 3;// 3
